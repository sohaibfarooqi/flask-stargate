...
class ResourceManager():
	...
	def create_resource_blueprint(self, name, model, methods=READONLY_METHODS,
		...
		self._add_route(blueprint, collection_url, view_func=api_view, methods=collection_methods)
		self._add_resource(collection_name, blueprint, serializer, deserializer, primary_key)

		return blueprint

	def _add_resource(self,collection_name, blueprint, serializer, deserializer, primary_key):
		self.registered_apis[model] = RESOURCE_API_INFO(collection_name, blueprint.name,
												serializer, deserializer, primary_key)
	
	def _add_route(self, blueprint, endpoint, view_func, _methods):
		add_rule = blueprint.add_url_rule
		add_rule(endpoint, view_func=view_func,
					methods=_methods)
	
	def _add_view(_apiname, session, model, decorators_, primary_key_, atmr, page_size_, max_page_size_, serializer_, deserializer_, includes_):
		return CollectionAPI.as_view(apiname, 
									session, 
									model,
